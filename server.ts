...
async function startServer() {
  ...
  // Note Endpoints
  app.get('/api/papers/:id/notes', (req: Request, res: Response) => {
    const note = db.getNoteForPaper(req.params.id);
    res.json(note || { content: '', title: '' });
  });

  app.post('/api/papers/:id/notes', (req: Request, res: Response) => {
    const { title, content, userId } = req.body;
    const note = db.createOrUpdateNote(req.params.id, title || 'Study Notes', content || '');
  ...
  app.get('/api/papers/:id/flashcards', async (req: Request, res: Response) => {
    ...
    let cards = db.getFlashcards(paperId);
  ...
  app.put('/api/flashcards/:cardId/difficulty', (req: Request, res: Response) => {
    const { difficulty } = req.body;
    const card = db.updateFlashcardDifficulty(req.params.cardId, difficulty);
    if (!card) {
  ...
  app.get('/api/papers/:id/quiz', async (req: Request, res: Response) => {
    const paperId = req.params.id;
    let quizzes = db.getQuizzes(paperId);
  ...
  app.put('/api/quizzes/:quizId/score', (req: Request, res: Response) => {
    const { score, userId } = req.body;
    const quiz = db.submitQuizScore(req.params.quizId, score);
    if (!quiz) {
      return res.status(404).json({ error: 'Quiz not found' });
    }

    const paper = db.getPaper(quiz.paperId);
    db.addActivity({
      userId: userId || 'u-1',
      type: 'quiz',
      paperTitle: paper ? paper.title : 'Research Paper',
  ...
  app.post('/api/chats/:id/messages', async (req: Request, res: Response) => {
    ...
    db.saveChatMessages(chatId, updatedMessages);

    // 2. Add Activity
    let activePaperTitle = 'Multi-Paper Synthesis';
    if (chat.paperId !== 'all') {
      const p = db.getPaper(chat.paperId);
    ...
    try {
      ...
      if (chat.paperId === 'all') {
        ...
        // Single-paper chat!
        const paper = db.getPaper(chat.paperId);
        if (!paper) {
      ...
      db.saveChatMessages(chatId, finalMessages);
      res.json(aiMsg);
  ...
  app.get('/api/reviews', (req: Request, res: Response) => {
    res.json(db.getLiteratureReviews());
  });
  ...
  app.post('/api/reviews', async (req: Request, res: Response) => {
    ...
    const papers = db.getPapers().filter(p => paperIds.includes(p.id));
    if (papers.length === 0) {
    ...
    try {
      ...
      db.createLiteratureReview(review);
  ...
  app.delete('/api/reviews/:id', (req: Request, res: Response) => {
    db.deleteLiteratureReview(req.params.id);
    res.json({ success: true });
  });

  // Citations Saved
  app.get('/api/citations', (req: Request, res: Response) => {
    res.json(db.getSavedCitations());
  });

  app.post('/api/citations', (req: Request, res: Response) => {
  ...
  app.delete('/api/citations/:id', (req: Request, res: Response) => {
    db.deleteSavedCitation(req.params.id);
    res.json({ success: true });
  });
  ...
  app.get('/api/help', (req: Request, res: Response) => {
    res.json({
      topics: [
        {
