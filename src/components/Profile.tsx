...
export default function Profile({ user, onOpenPaper, onUserUpdate }: ProfileProps) {
  ...
  const [editBio, setEditBio] = useState(user.bio || 'Academic researcher specializing in machine learning, information retrieval, and cognitive science.');
  ...
  const handleEditClick = () => {
    ...
    setEditUniversity(user.university || 'Stanford University');
    setEditDepartment(user.department || 'Computer Science');
    setEditBio(user.bio || 'Academic researcher specializing in machine learning, information retrieval, and cognitive science.');
  ...
  const handleSaveClick = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success && onUserUpdate) {
        ...
        setSuccessMsg('Academic profile successfully updated and synchronized!');
        setIsEditing(false);
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto text-left" id="profile_root">
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6" id="profile_hero_section">
        ...
        {/* COMPACT TOP METADATA SUMMARY */}
        <div className="text-center md:text-right text-xs text-slate-400 font-mono space-y-1">
          <div>ID: {user.id}</div>
          {user.username && <div>@{user.username}</div>}
        </div>
      </div>

      {/* 2. PROFILE INFORMATION CARD (With Edit Profile mode) */}
      <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-6" id="profile_info_card">
        <div className="flex items-center justify-between border-b border-slate-100 pb-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">Manage your active university credentials and workspace bio.</p>
          </div>
          {!isEditing && (
            <button
          ...
          <form onSubmit={handleSaveClick} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="space-y-3">
                ...
                {editAvatar && (
                  <button
                    ...
                    className="w-full text-center py-1.5 text-xs font-bold text-blue-600 hover:text-blue-700 transition-colors"
                  >
                    Change Photo
                  </button>
                )}
              ...
              <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {/* University */}
                <div className="space-y-1.5">
                  <label className="block text-xs font-bold text-slate-600">University / Institution</label>
                  <input
                    type="text"
                ...
                <div className="sm:col-span-2 space-y-1.5">
                  <label className="block text-xs font-bold text-slate-600 font-sans">Research Bio</label>
                  <textarea
                    value={editBio}
                    onChange={(e) => setEditBio(e.target.value)}
                    placeholder="Tell us about your active academic focus, literature scope, and research goals..."
                    rows={3}
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all font-medium leading-relaxed resize-none"
                  />
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
            ...
            <div className="flex flex-col items-center text-center space-y-3">
              <img 
                src={user.avatar || 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&q=80&w=240'} 
                alt={user.name} 
                className="w-32 h-32 rounded-3xl object-cover ring-4 ring-slate-100 shadow-md"
                referrerPolicy="no-referrer"
              />
              <div>
                <span className="font-mono text-xs font-bold text-slate-400 uppercase tracking-wider block">@{editUsername}</span>
                <span className="text-xs text-slate-500 font-medium block capitalize mt-1">{user.role}</span>
              </div>
            </div>

            {/* Right Col: Personal details fields */}
            <div className="md:col-span-2 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-xs">
                
                {/* Full Name */}
                <div className="space-y-1">
                  <span className="text-[10px] text-slate-400 uppercase font-mono font-bold tracking-wider block">Full Name</span>
                  <div className="text-slate-800 font-bold">{user.name}</div>
                </div>

                {/* Email */}
                <div className="space-y-1">
                  <span className="text-[10px] text-slate-400 uppercase font-mono font-bold tracking-wider block">Academic Email</span>
                  <div className="text-slate-800 font-bold">{user.email}</div>
                </div>

                {/* University */}
                <div className="space-y-1">
                  <span className="text-[10px] text-slate-400 uppercase font-mono font-bold tracking-wider block flex items-center gap-1">
                    <GraduationCap className="w-3.5 h-3.5 text-slate-400" />
                    University
                  </span>
                  <div className="text-slate-800 font-bold">{user.university || 'Stanford University'}</div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-slate-800 font-bold">{user.department || 'Computer Science'}</div>
                </div>
              ...
              <div className="space-y-1.5 border-t border-slate-100 pt-4 text-xs">
                ...
                <p className="text-slate-600 font-medium leading-relaxed">
                  {user.bio || 'Academic researcher specializing in machine learning, information retrieval, and cognitive science.'}
                </p>
              </div>
